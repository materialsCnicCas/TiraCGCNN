...
from model.modified_cgcnn import CGCNN
from model.gcn import SimpleGCN
from model.mcgcnn import iCGCNN
from jarvis.db.jsonutils import dumpjson
import json
import pprint
from model.modified_cgcnn import ZeroInflatedGammaLoss
import os
...
def train_dgl(
    ...
    checkpoint_dir: Path = Path("./output.checkpoint_200.pt"),
    ...
    if not train_val_test_loaders:
        ...
        train_data = train_val_test_loaders[4]
    ...
    if config.distributed:
        ...
        net = DDP(net,find_unused_parameters=True, device_ids=[rank % torch.cuda.device_count()])
    ...
    def log_results(engine):
        ...
        if config.progress:
            ...
            if not classification:
                ...
                print(f"Val_MAE: {vmetrics['mae']:.4f}", file=f)
                print(f"Train_MAE: {tmetrics['mae']:.4f}\n", file=f)
                f.close
                ...
                f = open("./output/out.txt", "a+")
                print(f"Train ROC AUC: {tmetrics['rocauc']:.4f}", file=f)
                print(f"Val ROC AUC: {vmetrics['rocauc']:.4f}\n", file=f)
                f.close
...
#         evaluator.add_event_handler(Events.EPOCH_COMPLETED, es_handler)
    ...
    if config.n_early_stopping is not None:
        ...
        def check_mae(engine):
            metrics = evaluator.state.metrics
            val_mae = metrics[my_metrics]
            if val_mae < 0.0075:
                trainer.terminate()  

        trainer.add_event_handler(Events.EPOCH_COMPLETED, check_mae)
    ...
    # train the model!
    if config.distributed:
        def train_with_epoch_callback(engine):
            epoch = engine.state.epoch
            train_sampler.set_epoch(epoch)

        trainer.add_event_handler(Events.EPOCH_STARTED, train_with_epoch_callback)
    ...
    if (
        ...
        f = open(os.path.join(config.output_dir, "test_mae.csv"),"w",)
        f.write("Test MAE:" + str(mean_absolute_error(np.array(targets), np.array(predictions))))
        f.close()
    if config.store_outputs and not classification:
        ...
        f.write("id,target,prediction\n")
        collate_fn = train_data.collate_line_graph
        train1_loader = DataLoader(train_data, batch_size=None, collate_fn=None, shuffle=False)
        print(len(train1_loader))
        with torch.no_grad():
            ids1 = train1_loader.dataset.ids
            for dat1, id1 in zip(train1_loader, ids1):
                g1, lg1, target1 = dat1
                out_data1 = net([g1.to(device), lg1.to(device)])
                out_data1 = out_data1.cpu().numpy().tolist()
                target1 = target1.cpu().numpy().flatten().tolist()
                if len(target1) == 1:
                    target1 = target1[0]
                    
                f.write("%s, %6f, %6f\n" % (id1, float(target1), float(out_data1)))
        f.close()
