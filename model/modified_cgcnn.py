...
from model.utils2 import BaseSettings
...
class CGCNNConvFull(nn.Module):
    ...
    def combine_edge_features(self, edges):
        ...
        concatenate source and destination node features with edge features将源结点要素和目标结点要素与边要素连接起来
        then apply the edge update modulated by the edge interaction model然后应用由边缘交互模型调制的边缘更新
        """
...
class CGCNNConv(nn.Module):
    ...
    def forward(
        ...
        m = torch.tanh(h_f) * F.softplus(h_s)
        g.edata["m"] = m
